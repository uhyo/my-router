...
var State = (function () {
    ...
    //go path
    State.prototype.go = function (segs, requirements) {
        ...
        if (seg[0] !== ppx) {
            ...
            for (var i = 0, table = this.staticTable, l = table.length; i < l; i++) {
                ...
                if (e.seg === seg) {
                    //match
                    fl = true;
                    result.push.apply(result, e.next.go(segs.slice(1), newrequirements));
                    break;
                }
            }
        }
        ...
        if (fl === false) {
            ...
            var p = seg[0] === ppx ? this.options.patterns[seg] : null;
            ...
            result.push.apply(result, res.go(segs.slice(1), newrequirements));
        }
        return result;
    };
    //match path
